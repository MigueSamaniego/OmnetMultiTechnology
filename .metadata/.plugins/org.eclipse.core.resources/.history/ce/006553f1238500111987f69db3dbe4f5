#include "AppBusTCP_R_StateM_B.h"
...
Define_Module(AppBusTCP_R_StateM_B);

bool AppBusTCP_R_StateM_B::initialized = false;
// --------------------------------------------------------------]
// --------------- Data Concatenator ----------------------------]
// --------------------------------------------------------------]
...
std::string AppBusTCP_R_StateM_B::buildDataString(const std::vector<double>& values)
{
...
int AppBusTCP_R_StateM_B::getBufferBytes(const std::vector<std::string>& buffer) const
{
    int total = 0;
...
AppBusTCP_R_StateM_B::AppBusTCP_R_StateM_B()
{
    // Begin flags
    ...
    Filter_EMA = 0.0;
    Adaptative_TCP = 0;
    smoothing_Factor = 0.8; // response slowly
...
void AppBusTCP_R_StateM_B::initialize(int stage)
{
    // CALL INIZIALITATION BASE
    veins::VeinsInetApplicationBase::initialize(stage);

    EV_INFO << "App BUS with Re-Try msg from server app AppBusTCP_R_StateM_B"<<endl;
    ...
    if (stage == INITSTAGE_APPLICATION_LAYER) {
        ...
        Cost_Data_wifi = registerSignal("Cost_Data_wifi_signal");
        Cost_Data_lte = registerSignal("Cost_Data_lte_signal");

        GW_Battery_signal = registerSignal("GW_Battery_level");
        ...
        this->GW_Battery_Capacity_mAh = par("GW_Battery_Capacity_mAh").doubleValue();
        this->GW_Battery_Level_Start = par("GW_Battery_Level_Start").intValue();
        if(GW_Battery_Level_Start > 100){
            GW_Battery_Level_Start = 100;
        }if(GW_Battery_Level_Start < 0){
        ...
        this->Strategy_Switching = par("Strategy_Switching").intValue();
...
// ----------------------------------------------------------------------
void AppBusTCP_R_StateM_B::receiveSignal(cComponent *source, simsignal_t signalID, long l, cObject *details)
{
...
void AppBusTCP_R_StateM_B::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AppBusTCP_R_StateM_B::receiveSignal(cComponent *source, simsignal_t signalID, double d, cObject *details)
{
...
bool AppBusTCP_R_StateM_B::startApplication()
{
...
bool AppBusTCP_R_StateM_B::stopApplication()
{
    if (Control_Task_Timer) {
...
void AppBusTCP_R_StateM_B::finish()
{
    ...
    cModule *localTcp = host->findModuleByPath(".tcp");
    if (localTcp) {
        localTcp->unsubscribe(L4_ACK_iD, this);
        EV_INFO << "Cancelada suscripción de L4_ACK_iD en " << localTcp->getFullPath() << endl;
    }

}

void AppBusTCP_R_StateM_B::processPacket(std::shared_ptr<inet::Packet> pk)
{
...
void AppBusTCP_R_StateM_B::handleMessage(cMessage *msg)
{
    ...
    if (msg->isSelfMessage()) {
            ...
            if (msg == Control_Task_Timer) {
                ...
                if(Battery_Available > 0.0){
                    ...
                    if (SignStateConnectAP == 0) { // GW UNSUBSCRIBE >> try to subscriber
                        ...
                        //********** Start GPS get Data 1000ms *********************************/
                        timer_Control_GPS_Data = 0;

                        //********* Start Collection Data 5000ms *******************************/
                        timer_Control_Data_Emission = 0;

                        //********* Start Get State Vehicle 1000ms *****************************/
                        timer_Control_Data_Vehicle = 0;

                        //********* Start Send Data to Cloud 30000ms *****************************/
                        time_threshold_send_data = 900000;// > 2h
                        timer_Control_Send_Data = 0; // long time until process, it can be update in the next step
                        ...
                        if(event_accident){
                            ...
                            if(timer_accident == accident_start){
                                ...
                                EV_ERROR << "accident detected" << endl;
                            ...
                            if(timer_accident == accidentDuration){
                                ...
                                event_accident = false; // finish with event
                            }
                        ...
                        // ----------------------------------------------------------------------------------------------------
                        // ---------------------------- GW MODULE -------------------------
                        double average_GW = 46.13;
                        double GW_MICRO_INA_Consumption_temp = 0.0;

                        GW_MICRO_INA_Consumption_temp = (average_GW+(normal(0, 1)/2)) * (10.0/3600000.0);

                        //emit(Consumption_WIFI,GW_MICRO_INA_Consumption_temp);
                        GW_MICRO_INA_Consumption += GW_MICRO_INA_Consumption_temp;   // This increase any time


                        // ---------------------------- GPS MODULE -------------------------
                        double average_gnss = 53.93;
                        double GNSS_Consumption_temp = 0.0;

                        GNSS_Consumption_temp = (average_gnss+(normal(0, 1)/10)) * (10.0/3600000.0);

                        //emit(Consumption_LTE,GNSS_Consumption_temp);
                        GNSS_Consumption += GNSS_Consumption_temp;
                        ...
                        // Average = 3.26mA
                        // Event tx msg = 7.41mS
                        // Event Wait replay = 289.22mS
                        // Event check data = 5.45mS
                        // Event prepare next request = 23mS

                        double TimeTxRx = 1.5;
                        double T_E_tx_msg = 7.41;
                        //double T_E_wait_reply = 289.22;// THIS IS CALCULATE USING THE LENGTH OF MESSAGE
                        double T_E_check_msg = 5.45;// THIS IS DELAY BEATWEEN PACKETS MTU BLE, the time minimmun is 5ms
                        double T_E_prepare_next_msg = 23.0;
                        double I_Average = 3.26;
                        double I_radio_Tx = 19.0;
                        double I_radio_Rx = 23.0;
                        double BLE_Consumption_temp = 0.0;

                        if(flag_Rx_BLE > 0){

                            timer_Machine_BLE += 10;// time each cicle

                            if(State_Machine_BLE == 0){// tx data
                                BLE_Consumption_temp = (I_radio_Tx*(TimeTxRx/3600000))+(I_Average*((10.0-TimeTxRx)/3600000));
                                State_Machine_BLE++;
                            }else if(State_Machine_BLE == 1){// rx data
                                BLE_Consumption_temp = (I_radio_Rx*(TimeTxRx/3600000))+(I_Average*((10.0-TimeTxRx)/3600000));
                                State_Machine_BLE++;
                            }else if((timer_Machine_BLE >= (timer_TxRx_BLE+T_E_tx_msg+T_E_check_msg+T_E_prepare_next_msg))&&(State_Machine_BLE == 2)){
                                BLE_Consumption_temp = ((I_Average+(normal(-3, 1)/10))*(10.0/3600000));// finally cicle
                                State_Machine_BLE = 0;
                                flag_Rx_BLE--;
                                timer_TxRx_BLE = 0.0;
                                timer_Machine_BLE = 0;
                            }else{
                                BLE_Consumption_temp = ((I_Average+(normal(-3, 1)/10))*(10.0/3600000));// consumption average

                                if((State_Machine_BLE == 2)&&(Packets_Machine_BLE > 0)){
                                    State_Machine_BLE = 1;
                                    Packets_Machine_BLE--;
                                    timer_Machine_BLE = 0;
                                }
                            ...
                            BLE_Consumption_temp = ((I_Average+(normal(-3, 1)/10))*(10.0/3600000));
                        }
                        //emit(Consumption_WIFI,BLE_Consumption_temp);
                        BLE_Consumption += BLE_Consumption_temp;

                        // ---------------------------- XBEE MODULE -------------------------
                        double TimeTxRx_XBee = 1.5;
                        double T_E_tx_msg_XBee = 7.99;
                        //double T_E_wait_reply_XBee = 282.45;// THIS IS CALCULATE USING THE LENGTH OF MESSAGE
                        double T_E_check_msg_XBee = 1.28;// THIS IS DELAY BEATWEEN PACKETS MTU BLE, the time minimmun is 5ms
                        double T_E_prepare_next_msg_XBee = 6.18;
                        double I_Average_XBee = 12.74;
                        double I_radio_Tx_XBee = 35.3;
                        double I_radio_Rx_XBee = 35.0;
                        double XBEE_Consumption_temp = 0.0;

                        if(flag_Rx_XBee > 0){

                            timer_Machine_XBee += 10;// time each cicle

                            if(State_Machine_XBee == 0){// tx data
                                XBEE_Consumption_temp = (I_radio_Tx_XBee * (TimeTxRx_XBee/3600000.0))+(I_Average_XBee * ((10.0-TimeTxRx_XBee)/3600000.0));
                                State_Machine_XBee++;
                            }else if((timer_Machine_XBee >= (timer_TxRx_XBee + T_E_tx_msg_XBee))&&(State_Machine_XBee == 1)){
                                XBEE_Consumption_temp = (I_radio_Rx_XBee * (TimeTxRx_XBee/3600000.0))+(I_Average_XBee * ((10.0-TimeTxRx_XBee)/3600000.0));
                                State_Machine_XBee++;
                            }else if((timer_Machine_XBee >= (timer_TxRx_XBee + T_E_tx_msg_XBee + T_E_check_msg_XBee + T_E_prepare_next_msg_XBee))&&(State_Machine_XBee == 2)){
                                XBEE_Consumption_temp = ((I_Average_XBee+(normal(-3, 1)/10)) * (10.0/3600000.0));
                                State_Machine_XBee = 0;
                                flag_Rx_XBee--;
                                timer_Machine_XBee = 0;
                                timer_TxRx_XBee = 0.0;
                            }else{
                                XBEE_Consumption_temp = ((I_Average_XBee+(normal(-3, 1)/10)) * (10.0/3600000.0));
                            }
                        ...
                        //emit(Consumption_LTE,XBEE_Consumption_temp);
                        XBEE_Consumption += XBEE_Consumption_temp;
                        ...
                        if(timer_update_GW_Batery >= 100){
                            ...
                            Battery_Available = 0.0; // insure level of battery
                        ...
                        if(control_all_timers >= delay_sdcard){
                            ...
                            /*************************************************************
                             ******* interface control must be continuous ****************
        ...
        //                            }
                            ...
                            /*************************************************************
                             ******* The socket control must be continuous ***************
                             *************************************************************
                             *************************************************************/
                            ...
                            if(socket_state_close){
                                ...
                                if(count_reTX >= (waiting_changing_x_10ms + 1)){
                                    ...
                                    if(count_reTX == (waiting_changing_x_10ms + 1)){
                                        ...
                                        if(Strategy_Switching==1){//wifi
                                            if(!stablishTCP(1)){
                                                EV_WARN << "ERROR CREATING NEW SOCKET, retrying..." << endl;
                                                count_reTX = 0;
                                            }
                                        }else if(Strategy_Switching==2){// LTE
                                            if(!stablishTCP(0)){
                                                EV_WARN << "ERROR CREATING NEW SOCKET, retrying..." << endl;
                                                count_reTX = 0;
                                            }
                            ...
                            if (timer_Control_GPS_Data >= 100) {
                                ...
                                timer_Control_GPS_Data = 0; // restart timer
                            }
                            ...
                            if (timer_Control_Data_Emission >= 500) {
                                ...
                                // -------- OTHER TYPE SENSORS ----------------
                                 std::vector<double> sensor = {
                                         (double)Priority_3, deadline_on_SERVER, co2, co, nox, pmx
                                 };
                                 ...
                                 timer_Control_Data_Emission = 0; // restart timer
                            ...
                            if (timer_Control_Data_Vehicle >= 100) {// control of traffic congestion
                                ...
                                // -------- OTHER TYPE SENSORS ----------------
                                 std::vector<double> sensor = {
                                         (double)Priority_wifi, deadline_on_SERVER, speed, accel, Decaccel, fuel, time_trip
                                 };
                            ...
                            if ((timer_Congestion_traffic >= 50)&&(!accident_detected_app)) {// each 500ms

                                // state of detected speed
                                // 0 moving car
                                // 1 speed medium
                                // 2 stop detected
                                ...
                                if(detected_traffic_state==0){
                                    ...
                                    if(speed <= 10.8) {//15
                                        ...
                                        if(timer_congestion_state >= sensibility){
                                            detected_traffic_state = 1;
                                            timer_congestion_state = 19; // just for activation
                                        }
                                    ...
                                    if(speed > 10.8) {
                                        ...
                                        if(timer_congestion_state >= periodic_nitification){
                                            timer_congestion_state = 0;
                                            ...
                                            deadline_on_SERVER = simTime().dbl();
                                             ...
                                             std::vector<double> event_traffic = {
                                                     (double)Priority_2, deadline_on_SERVER, (double)(periodic_nitification/2), lat, lon
                                             };
                                             ...
                                             if (state_buff) {
                                                 ...
                                                 EV_INFO << "SAVING buffer fill" << endl;
                                                 counter_msg = 0;
                            ...
                            //if(msg_emergency_pending) send_data = true;

                            bool msg_emergency_active = false;
                            ...
                            if(accident_detected_app){
                                ...
                                if((timer_Control_Accident == 0)||(timer_Control_Accident == 300)){ // start or 3seconds after
                                    state_of_accident = 1;
                                    //send_data = true;
                                    //msg_emergency_pending = true;
                                    msg_emergency_active = true;

                                }

                                if (timer_Control_Accident % 1000 == 0){// send each 10s
                                    state_of_accident = 2;
                                    //send_data = true;
                                    //msg_emergency_pending = true;
                                    msg_emergency_active = true;
                                ...
                                if(msg_emergency_active){
                                    ...
                                    deadline_on_SERVER = simTime().dbl();
                                     ...
                                     std::vector<double> accident_traffic = {
                                             (double)Priority_1, deadline_on_SERVER, (double)state_of_accident, lat, lon
                                     };
                                ...
                                timer_Control_Accident++;

                            }else if(timer_Control_Accident > 0){

                                state_of_accident = 3;
                                //send_data =true;
                                //msg_emergency_pending = true;
                                timer_Control_Accident = 0; // it not allow inside on this condition more

                            }

                            // ---------------------------------------------------------------------------------------------------------------
                            // ****************************** pain attention, the code continue after  ***************************************
                            // ************************** the function to sent the message not importants ************************************
                            // ---------------------------------------------------------------------------------------------------------------
                            ...
                            if(Strategy_Switching == 1){
                                ...
                                if(socket_ready){
                                    take_time_when_find_expired_data = true; // use to turn ON the interface LTE
                                    //EV_ERROR << "strategy 2: "<< endl;
                                    if (timer_Control_Send_Data >= time_threshold_send_data){

                                        deadline_start = simTime()-1;
                                        deadline_until = 6000; // take all data to send
                                ...
                                if(ConnectionToAP){ // WIFI available
                                    ...
                                    if(socket_ready){
                                        ...
                                        if (timer_Control_Send_Data >= time_threshold_send_data){
                                                ...
                                                deadline_start = simTime()-1; // update this time each call function is sufficient to empty data from SDCARD
                                                deadline_until = 6000;// maximo para botar todos los datos
                                            ...
                                            if(socket_ready){
                                                ...
                                                if (timer_Control_Send_Data >= time_threshold_send_data){
                                                    EV_ERROR <<" Send data por LTE"<< endl;
                                                       //****** Sent Message to Cloud *********************
                                                       if(buffer_penalty != -1){
                                                           sendDataToCloud_Strategy_5("lte");
                                                       }else{
                                                           sendDataToCloud("lte"); // prepare and send message
                                                       }
                                                       flag_tx_lte = true;
                                        ...
                                        if(socket_ready){
                                            ...
                                            if (timer_Control_Send_Data >= time_threshold_send_data){
                                                EV_ERROR <<" Send data por wifi"<< endl;
                                                    ...
                                                    deadline_until = 600.0;

                                                    //****** Sent Message to Cloud *********************
                                                    if(buffer_penalty != -1){
                                                        sendDataToCloud_Strategy_5("wifi");
                                                    }else{
                                                        sendDataToCloud("wifi"); // prepare and send message
                                                    }
                                                    flag_tx_wifi = true;
                                ...
                                if(ConnectionToAP){ // WIFI available
                                    ...
                                    if(socket_ready){
                                        ...
                                        if (timer_Control_Send_Data >= time_threshold_send_data){
                                           ...
                                           timer_Control_Send_Data = 0;
                                           emit(model_cost_offloading_wifi, 0);// actualiza estado de salida del modelo 2
                                       }
                                    ...
                                    //check_timers_expired = 105; // USED TO CHECK IN THE NEXT CYCLE WHEN WIFY IS LOSE
                                    take_time_when_find_expired_data = false; // reset this variable while it has WIFI, in this case i must sento all data
                                    Function_State = 0; //indica que debe calcular el modelo antes de actuar
                                    timer_fuction_multi_objetive = timer_Control_Send_Data; //continua con el conteo para el timer del calculo del modelo



                                }// quie es mi verdadero problema, TX por LTE o mantener en memoria, cual me combiene?
                                else{
...
//                                            }
                                        ...
                                        EV_ERROR <<" ESTADO DEL SOCKET LTE" << ConnectionToAP << " - " <<  take_time_when_find_expired_data << " - " << socket_ready << " -deadlineStart: " << deadline_start << " -deadlineStop: " << deadline_until << endl;
                            ...
                            control_all_timers++;
                            EV_INFO << "number retardo activado, control_all_timers, delay_sdcard: " << " " << control_all_timers << " " << delay_sdcard << endl;
                        }
                    ...
                    if (update_timer) {
                         ...
                         if((SignStateConnectAP == 3)&&(control_all_timers >= delay_sdcard)){
                             ...
                             emit(timeLTEWorking, (double)(((timer_LTE_Average)*10)/1000));// time on seconds
                             ...
                             // Things Mobile è un operatore IoT/M2M reale molto utilizzato in Italia per la telemetria e i sistemi di trasporto intelligenti (ITS)
                             // 0,10 € / MB (a consumo)
                             // https://www.thingsmobile.com/it/business

                             // conversione nel  networking
                             // 1KB = 1024 B
                             // 1MB = 1024 * 1024B = 1.048.576 B
                             // Coste by Byte = (0.10 / 1.048.576 B) = 0,0000000954
                             //-------------------------------------------------------------
                             // ******* consideretion header MQTT + TCP + IP ***************
                             // ------------------------------------------------------------
                             // Total de Overhead MQTT: Aproximadamente 21 Bytes por paquete.
                             // Cabecera TCP: 20 Bytes
                             // Cabecera IPv4: 20 Bytes

                             Cost_LTE = long_Bytes_LTE_Header * 0.0000000954;
                             Cost_WiFi = 0; // IT IS FREE

                             emit(Cost_Data_wifi, Cost_WiFi);
                             emit(Cost_Data_lte, Cost_LTE);
                    ...
                    //if(!GW_Battery_died){
                    //    scheduleAt(simTime() + SimTime(1, SIMTIME_S), Control_Task_Timer);   // just to notification GW BATTERY FINISHED
                    //    GW_Battery_died = true;
                    //}
...
void AppBusTCP_R_StateM_B::subscriptionSignalState(){
...
void AppBusTCP_R_StateM_B::sendDataToCloud(const std::string& interface_output)
{
    ...
    if(Vehicle_With_Interface){
        ...
        if (outputBuffer.isEmpty()) {
            ...
            if(data_priority_on_sd >= 20){
                if(Strategy_Switching == 1){
                    auto batch = sdcard.popBatch(20, deadline_start, deadline_until, 1);// wifi
                    if(!batch.empty())
                        outputBuffer.load(batch);
                }else if(Strategy_Switching == 2){
                    auto batch = sdcard.popBatch(20, deadline_start, deadline_until, 1);// LTE pero mandamos 1 como su estubiera conectado a WIFI para mandar todos los datos
                    if(!batch.empty())
                        outputBuffer.load(batch);
                }else if(Strategy_Switching == 3){
...
inet::Coord AppBusTCP_R_StateM_B::convertXYtoLatLon(inet::Coord pos)
{
...
bool AppBusTCP_R_StateM_B::stablishTCP(bool Use_Netwok)
{
...
void AppBusTCP_R_StateM_B::printSocketInfo() {
...
void AppBusTCP_R_StateM_B::socketDataArrived(inet::TcpSocket *socket, inet::Packet *packet, bool urgent) {
...
void AppBusTCP_R_StateM_B::socketAvailable(inet::TcpSocket *socket, inet::TcpAvailableInfo *availableInfo) {
    EV_INFO << "socket error 7" << endl;
    delete availableInfo; // Liberar memoria si no se usa
}

void AppBusTCP_R_StateM_B::socketEstablished(inet::TcpSocket *socket) {
...
void AppBusTCP_R_StateM_B::socketStatusArrived(inet::TcpSocket *socket, inet::TcpStatusInfo *statusInfo) {
...
void AppBusTCP_R_StateM_B::socketDeleted(inet::TcpSocket *socket) {
...
/*int AppBusTCP_R_StateM_B::FuctionMultiObjetive(double alpha, double beta, double gamma_w, int coefficient, int threshold_memmory) {
    ...
    if (val.D_t > 0) {
        ...
        P_t_1 = 1.0 / (900.0+300.0);

    }

    EV_WARN << "Penalty: " << P_t << " penality_1: " << P_t_1 << endl;

    // 2. CÁLCULOS DE ENERGÍA Y COSTO FINANCIERO
    double Consumption_GW_sleep = ((300000)/3600000.0) * (116.081 + 0.8 + 13.14);

    EV_WARN << "Consumo en sleep: " << Consumption_GW_sleep << endl;

    long Tx_phase = 0, Rx_phase = 0, Normal_phase = 0, time_on_sleep = 0;
    if (val.D_t <= (300000/100)) {

        Tx_phase = val.D_t*5;
        Rx_phase = val.D_t*2;
        Normal_phase = (val.D_t*3);// + 2500; // tiempo para entrar en sleep
        time_on_sleep = ((300000) - (Tx_phase + Rx_phase + Normal_phase ));// restamos el timepo de Tx y Rx y un tiempo
    ...
    double MAX_FINANCIAL_COST = ((300000/100)*100) * 0.0000000954; // asumimos que en la ventana que usamos de 5min y que cada paquete puede tener 100B max
    ...
    // ENCONTRAR EL MAXIMO
    long max_buffer_fill = 0;// no puede ser 0 cancela la penalizacion
    ...
    EV_WARN << "penali: " << P_t << " penaly_1: " << P_t_1 << endl;


    double MAX_PENALTY_SCORE = (1.0/0.001) * (300000/100); // Urgencia max * Max datos posibles por buffer
    ...
    if(Consumption_GW_lte >= MAX_ENERGY_CONSUMPTION_LTE){
        Consumption_GW_lte = MAX_ENERGY_CONSUMPTION_LTE;
    }

    double energy_norm_1 = Consumption_GW_lte / MAX_ENERGY_CONSUMPTION_LTE;

    double mem_norm_1 = (P_t * val.D_t) / MAX_PENALTY_SCORE;

    model[1] = (alpha * cost_norm_1) + (beta * energy_norm_1) + 0;//(gamma_w * mem_norm_1);
...
/*int AppBusTCP_R_StateM_B::FuctionMultiObjetive(double alpha, double beta, double gamma_w, int coefficient, int threshold_memmory, double windows) {
    ...
    for(int n=0; n<=4; n++){
        ...
        double penaly_saturation = (100.0 - ((data_on_buffer[n]*100)/max_buffer_fill));
        //EV_ERROR << "saturacion: " << penaly_saturation << endl;
        //EV_ERROR << "ocupacion: " << data_on_buffer[n] << endl;

        if(penaly_saturation <= threshold_memmory){
            buffer_penalty = n;
            P_t = true;
        }
    }
...
/*int AppBusTCP_R_StateM_B::FuctionMultiObjetive(double alpha, double beta, double gamma_w, int coefficient, int threshold_memmory, double windows) {
    ...
    model[0] = (beta * energy_norm_0) + (gamma_w * (int)P_t);
    //EV_ERROR << "modelo 0 : " << model[0] << " par 1: " << (beta * energy_norm_0) << " par 2: "<< (gamma_w * (int)P_t) << endl;
...
/*int AppBusTCP_R_StateM_B::FuctionMultiObjetive(double alpha, double beta, double gamma_w, int coefficient, int threshold_memmory, double windows) {
    ...
    double MAX_ENERGY_CONSUMPTION_LTE = ((windows*2)+(windows*0.2))*((((5)/3600000.0) * (116.081 + 0.8 + 500)) +
                                                          (((2)/3600000.0) * (116.081 + 0.8 + 300)) +
                                                          (((93)/3600000.0) * (116.081 + 0.8 + 80))); // Ajusta al pico máximo de Ah que podría gastar LTE en 300s
