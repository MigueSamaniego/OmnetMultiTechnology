...
Define_Module(AppBusTCP_R_StateM);

bool AppBusTCP_R_StateM::initialized = false;
// --------------------------------------------------------------]
// --------------- Data Concatenator ----------------------------]
// --------------------------------------------------------------]
...
std::string AppBusTCP_R_StateM::buildDataString(const std::vector<double>& values)
{
...
int AppBusTCP_R_StateM::getBufferBytes(const std::vector<std::string>& buffer) const
{
    int total = 0;
...
AppBusTCP_R_StateM::AppBusTCP_R_StateM()
{
    // Begin flags
...
void AppBusTCP_R_StateM::initialize(int stage)
{
    // CALL INIZIALITATION BASE
    veins::VeinsInetApplicationBase::initialize(stage);

    EV_INFO << "App BUS with Re-Try msg from server app AppBusTCP_R_StateM"<<endl;
...
bool AppBusTCP_R_StateM::stopApplication()
{
    if (Control_Task_Timer) {
...
void AppBusTCP_R_StateM::processPacket(std::shared_ptr<inet::Packet> pk)
{
        ...
        if (outputBuffer.isEmpty()) {
            ...
            while(packet_lose){
                ...
                if (outputBuffer.isEmpty()) {
                    ...
                    double fresh_data = 0.0;

                    if(tipoMensaje == 0){// priority 1
                        fresh_data = (deadlineValue + 2.0) - simTime().dbl();
                    }else if(tipoMensaje == 1){// priority 2
                        fresh_data = (deadlineValue + 60.0) - simTime().dbl();
                    }else if(tipoMensaje == 2){// priority 3
                        fresh_data = (deadlineValue + 900.0) - simTime().dbl();
                    }else if(tipoMensaje == 3){
                        fresh_data = (deadlineValue + 9600.0) - simTime().dbl();
                    }else if(tipoMensaje == 4){
                        fresh_data = (deadlineValue + 9650.0) - simTime().dbl();
                    }

                    if(fresh_data > 0.0){// envio el dato y salgo del while
                        ...
                        EV_ERROR << "PACKET SENT to SERVER: "<< data << " " << fresh_data << endl;
                        ...
                        packet_lose = false; // termino el while

                    }else{// descartar el dato contar y leer el siguiente
                        // contar los paquetes descartados
                        EV_ERROR << "socket WIFI OK4.4.4.4: " << data << fresh_data << endl;
                        number_packet_lose++;
                        emit(Packet_lose_deadline, number_packet_lose);
                        number_bytes_lose += data.size();
                        emit(Byte_lose_deadline, number_bytes_lose);
                    }
...
void AppBusTCP_R_StateM::interfaceAvailable(){
...
inet::Coord AppBusTCP_R_StateM::convertXYtoLatLon(inet::Coord pos)
{
...
bool AppBusTCP_R_StateM::stablishTCP(bool Use_Netwok)
{
...
void AppBusTCP_R_StateM::printSocketInfo() {
