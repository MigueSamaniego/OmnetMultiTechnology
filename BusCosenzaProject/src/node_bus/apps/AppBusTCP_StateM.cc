...
Define_Module(AppBusTCP_StateM);

bool AppBusTCP_StateM::initialized = false;
// --------------------------------------------------------------]
// --------------- Data Concatenator ----------------------------]
// --------------------------------------------------------------]
...
std::string AppBusTCP_StateM::buildDataString(const std::vector<double>& values)
{
...
int AppBusTCP_StateM::getBufferBytes(const std::vector<std::string>& buffer) const
{
    int total = 0;
...
AppBusTCP_StateM::AppBusTCP_StateM()
{
    // Begin flags
...
void AppBusTCP_StateM::initialize(int stage)
{
    // CALL INIZIALITATION BASE
    veins::VeinsInetApplicationBase::initialize(stage);

    EV_INFO << "App BUS with Re-Try msg from server app AppBusTCP_StateM"<<endl;
    ...
    if (stage == INITSTAGE_APPLICATION_LAYER) {
        ...
        //EV_INFO << "Inicializando AppBusTCP_StateM en stage " << stage << endl;
...
// ----------------------------------------------------------------------
void AppBusTCP_StateM::receiveSignal(cComponent *source, simsignal_t signalID, long l, cObject *details)
{
...
bool AppBusTCP_StateM::startApplication()
{
...
bool AppBusTCP_StateM::stopApplication()
{
    if (Control_Task_Timer) {
...
void AppBusTCP_StateM::processPacket(std::shared_ptr<inet::Packet> pk)
{
...
void AppBusTCP_StateM::handleMessage(cMessage *msg)
                ...
                //            << std::defaultfloat << endl;
                        ...
                        if (timer_Control_Send_Data >= time_threshold_send_data) {
                           ...
                           if(socket_ready){
                               ...
                               time_threshold_send_data = 1; // minimmun time (each 10ms)
...
void AppBusTCP_StateM::subscriptionSignalState(){
...
void AppBusTCP_StateM::sendDataToCloud(){
...
void AppBusTCP_StateM::interfaceAvailable(){
...
inet::Coord AppBusTCP_StateM::convertXYtoLatLon(inet::Coord pos)
{
...
bool AppBusTCP_StateM::stablishTCP(bool Use_Netwok)
{
...
void AppBusTCP_StateM::printSocketInfo() {
...
void AppBusTCP_StateM::socketDataArrived(inet::TcpSocket *socket, inet::Packet *packet, bool urgent) {
...
void AppBusTCP_StateM::socketAvailable(inet::TcpSocket *socket, inet::TcpAvailableInfo *availableInfo) {
    //EV_INFO << "socker available 1." << endl;
    delete availableInfo; // Liberar memoria si no se usa
}

void AppBusTCP_StateM::socketEstablished(inet::TcpSocket *socket) {
    EV_INFO << "Connexion TCP successful." << endl;
}

void AppBusTCP_StateM::socketPeerClosed(inet::TcpSocket *socket) {
...
void AppBusTCP_StateM::socketClosed(inet::TcpSocket *socket) {
...
void AppBusTCP_StateM::socketFailure(inet::TcpSocket *socket, int code) {
...
void AppBusTCP_StateM::socketStatusArrived(inet::TcpSocket *socket, inet::TcpStatusInfo *status) {
    EV_INFO << "socket status arrived" << endl;
    delete status;
}

void AppBusTCP_StateM::socketDeleted(inet::TcpSocket *socket) {
