...
AppBusDataCollectionTCP::AppBusDataCollectionTCP()
{
    ...
    SignStateConnectAP = false; // Usamos SignStateConnectAP como bandera de 'suscripción hecha'
...
bool AppBusDataCollectionTCP::startApplication()
{
    ...
    if (!Control_Task_Timer) {
        ...
        scheduleAt(simTime() + SimTime(100, SIMTIME_MS), Control_Task_Timer); // the GW REQUIRE MORE TIME TO (1s) TO RECOVER INITIALITATION VARIABLES VALUES
    }
    return true;
}
...
void AppBusDataCollectionTCP::handleMessage(cMessage *msg)
{
    if (msg == Control_Task_Timer) {
        ...
        if (!SignStateConnectAP) { // useful to know the state interface wifi (connected/disconnected)
            ...
            // if car is subscriber successfull we can start with control of interfaces
...
void AppBusDataCollectionTCP::subscriptionSignalState(){
    ...
    if (wlanAgent) {
        ...
        SignStateConnectAP = true; // flag to indicate subscription to signal 'stateAssociationSignalId' successful
    } else {
...
void AppBusDataCollectionTCP::controlInterface(){

    if(Vehicle_With_Interface){ // if the vehicle has interface
        ...
        auto packet = createPacket("accident_car_test");


        packet->insertAtBack(payload);

        /**********************************
         * update interface GW to use
         **********************************/
