...
//simsignal_t Ieee80211AgentSta::stateAssociationSignal = registerSignal("stateAssociation");
simsignal_t Ieee80211AgentSta::stateAssociationSignal = registerSignal("stateAssociationSignal");
...
//        sendDisassociateRequest();
        ...
        emit(stateAssociationSignal, (long)false);
        EV_INFO << "emit l2DisassociatedSignal loss enviada: " << endl;
    }
}
...
void Ieee80211AgentSta::processAssociateConfirm(Ieee80211Prim_AssociateConfirm *resp)
{
    if (resp->getResultCode() != PRC_SUCCESS) {
        ...
        if (prevAP.isUnspecified() || prevAP != resp->getAddress()) {
            ...
            EV_INFO << "emit l2AssociatedNewApSignal enviada: " << endl;
            emit(stateAssociationSignal, (long)true);
            prevAP = resp->getAddress();
        }
        else{
            emit(l2AssociatedOldApSignal, myIface);
            EV_INFO << "emit l2AssociatedOldApSignal enviada: " << endl;
        }
    }
}
