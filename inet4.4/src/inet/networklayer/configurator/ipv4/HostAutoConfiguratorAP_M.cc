...
#include "inet/networklayer/configurator/ipv4/HostAutoConfiguratorAP_M.h"
...
Define_Module(HostAutoConfiguratorAP_M);

void HostAutoConfiguratorAP_M::initialize(int stage)
{
...
void HostAutoConfiguratorAP_M::finish()
{
}

void HostAutoConfiguratorAP_M::handleMessageWhenUp(cMessage *apMsg)
{
}

void HostAutoConfiguratorAP_M::setupNetworkLayer()
{
    ...
    // Interface IPV4
    Ipv4Address addressBasePPP = Ipv4Address(par("addressBasePPP").stringValue());
    Ipv4Address netmaskPPP = Ipv4Address(par("netmaskPPP").stringValue());
    ...
    Ipv4Address myAddress = Ipv4Address(addressBase.getInt());
    Ipv4Address myAddressPPP = Ipv4Address(addressBasePPP.getInt() + uint32_t(host->getId()));
    ...
    //address test
    if (!Ipv4Address::maskedAddrAreEqual(myAddressPPP, addressBasePPP, netmaskPPP))
        throw cRuntimeError("Generated IP address PPP is out of specified address range");
...
void HostAutoConfiguratorAP_M::handleStartOperation(LifecycleOperation *operation)
{
...
void HostAutoConfiguratorAP_M::handleStopOperation(LifecycleOperation *operation)
{
}

void HostAutoConfiguratorAP_M::handleCrashOperation(LifecycleOperation *operation)
{
}
